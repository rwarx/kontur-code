...
/// <summary>
/// The questions the canvas and the inspector can ask on a person's behalf.
/// </summary>
/// <remarks>
/// Ported unchanged: both surfaces offer the same actions on the same selection, and two
/// copies of these sentences would drift apart. The graph context is attached separately by
/// <c>IGraphContextSource</c> during the normal context build.
/// </remarks>
public static class CanvasAiPrompts
{
    /// <summary>
    /// The question behind an action name, or an empty string for "Ask" - where the person
    /// types their own and the selection simply rides along.
    /// </summary>
